...
class Evaluator:
    ...
    def evaluate(self):
        ...
        # if True:
            ...
            # print(self.params.model_file.split('.')[0])
            sns_plot.get_figure().savefig(f'plots/GAT_2attn.png') 
            
        if self.params.ntags == 2:
        # if True:
            results = confusion_matrix(all_actual, all_predicted)
            print(results)
            df_cm = pd.DataFrame(results, index=[i for i in ["Satire","Trusted"]],
                                 columns=[i for i in ["Satire","Trusted"]])
            sns_plot = sn.heatmap(df_cm, annot=True, fmt='g')
            plt.yticks(rotation=45)
            # print(self.params.model_file.split('.')[0])
            sns_plot.get_figure().savefig(f'plots/GAT_2attn_2tag.png') 
