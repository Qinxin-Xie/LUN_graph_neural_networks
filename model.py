...
from layers import GraphConvolution, GraphAttentionLayer, DeepGraphConvolution
import matplotlib
matplotlib.use('Agg')
...
class Classify(torch.nn.Module):
    def __init__(self, params, vocab_size, ntags, pte=None):
        ...
        if params.encoder == 2:  # GCN model
            self.gcn2 = GraphConvolution(params.hidden_dim, params.node_emb_dim, params.dropout, act=F.relu)
            self.linear_transform = nn.Linear(in_features=params.node_emb_dim,
                                              out_features=ntags)                                    
        elif params.encoder == 3: # GAT model with 1 attention head
            ...
            self.linear_transform = nn.Linear(in_features=params.node_emb_dim,
                                              out_features=ntags)
        elif params.encoder == 4: # GAT model with 2 attention heads
            self.gcn2 = GraphAttentionLayer(params.hidden_dim, params.node_emb_dim, params.dropout, 0.2)
            self.attentions = [GraphAttentionLayer(params.hidden_dim, params.node_emb_dim, dropout=params.dropout,
            ...
            self.linear_transform = nn.Linear(in_features=params.node_emb_dim,
                                              out_features=ntags)
        elif params.encoder == 5:  # GAT model with 3 attention head
            self.gcn2 = GraphAttentionLayer(params.hidden_dim, params.node_emb_dim, params.dropout, 0.2)
            self.attentions = [GraphAttentionLayer(params.hidden_dim, params.node_emb_dim, dropout=params.dropout,
                                                   alpha=0.2, concat=True) for _ in range(3)]
            for i, attention in enumerate(self.attentions):
                self.add_module('attention_{}'.format(i), attention)

            self.out_att = GraphAttentionLayer(params.node_emb_dim * 3, params.node_emb_dim, dropout=params.dropout,
                                               alpha=0.2, concat=False)
            # Add the attention thingy
            self.linear_transform = nn.Linear(in_features=params.node_emb_dim,
                                              out_features=ntags)
        elif params.encoder == 6: # deepgcn
            self.gcn2 = DeepGraphConvolution(params.hidden_dim, params.node_emb_dim, params.dropout, act=F.relu)
            self.linear_transform = nn.Linear(in_features=params.node_emb_dim,
                                              out_features=ntags)
        else:
    ...
    def forward(self, input_sents, input_lens, adj=None, actual_sentence=None):
        ...
        if self.params.encoder == 2 or self.params.encoder == 6:
            ...
            # Simple max pool on all node representations
            h, _ = h.max(dim=0)
        elif self.params.encoder == 4 or self.params.encoder == 5:
            # Currently it's a dummy matrix with all edge weights one
            adj_matrix = np.ones((h.size(0), h.size(0))) if adj is None else adj
            # Setting link between same sentences to 0
            np.fill_diagonal(adj_matrix, 0)
